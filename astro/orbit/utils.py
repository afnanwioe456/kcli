...
def orbit_launch_window(orbit: Orbit, 
    ...
    orbit_plane_normal = orbit.h_vec.to_value(u.km ** 2 / u.s)
    n = orbit.attractor.angular_velocity.to_value(u.rad / u.s)
    ...
    if not search:
        return [res * u.s for res in result_launch_window]

    # 计算最佳发射窗口
    min_phase = min_phase.to_value(u.rad)
    max_phase = max_phase.to_value(u.rad)
